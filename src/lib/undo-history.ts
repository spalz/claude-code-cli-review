...
export function pushUndoState(fsPath: string, review: Snapshotable, preserveRedo = false): void {
    ...
    log.log(
        `undo-history: push undo, stack=${hist.undoStack.length}, hunks=[${hunkSummary(review.hunks)}] for ${fsPath}`,
    );
    updateContextKeys();
}
...
export function popUndoState(fsPath: string): ReviewSnapshot | undefined {
    ...
    log.log(
        `undo-history: pop undo, remaining=${hist.undoStack.length}, hunks=[${hunkSummary(snapshot.hunks)}] for ${fsPath}`,
    );
    updateContextKeys();
    return snapshot;
}
...
function updateContextKeys(): void {
    ...
    log.log(
        `undo-history: updateContextKeys — ccr.canUndoReview=${canUndo}, ccr.canRedoReview=${canRedo}, totalFiles=${histories.size}`,
    );
