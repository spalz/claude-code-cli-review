...
export async function applyTargetedHunkEdit(
	...
	logCat("content",`applyTargetedHunkEdit: deleting lines ${deleteStart}-${deleteEnd} in ${filePath} (docLines=${doc.lineCount})`);
	...
	try {
		...
		logCat("content",`applyTargetedHunkEdit: edit result=${editSuccess}, docLines after=${doc.lineCount}`);
		if (editSuccess) {
	...
	if (editSuccess) {
		...
		if (review) {
			...
			logCat("content",`applyTargetedHunkEdit: post-edit verify bufferMatch=${bufferMatch}, expectedLines=${review.mergedLines.length}, docLines=${doc.lineCount}`);
			if (!bufferMatch) {
				logCat("content",`applyTargetedHunkEdit: content mismatch after delete, falling back to full replace`);
			...
			if (doc.getText() === expected) {
				...
				logCat("content",`applyTargetedHunkEdit: buffer still mismatched after fallback, skipping decorations`);
			}
		}
	}
