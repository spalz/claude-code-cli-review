...
export async function addFile(
    ...
    if (
        !absFilePath ||
        absFilePath.endsWith("/") ||
        absFilePath.includes("\n") ||
    ...
    try {
        ...
        logCat(
            "file-add",
            `cannot read ${absFilePath}: ${(err as Error).message} — treating as deletion`,
        );
    ...
    if (originalContent === modifiedContent) {
        logCat(
            "file-add",
            `no change: original === modified for ${absFilePath} (${originalContent.length} chars)`,
        );
    ...
    logCat(
        "file-add",
        `${absFilePath}: changeType=${changeType}, original=${originalContent.length}chars, modified=${modifiedContent.length}chars`,
    );
    ...
    if (hunks.length === 0) {
        logCat(
            "file-add",
            `SKIP ${absFilePath}: computeDiff returned 0 hunks (original and modified may differ only in whitespace or formatting)`,
        );
        return;
    }
    ...
    const review = new FileReview(absFilePath, originalContent, modifiedContent, hunks, changeType);
    ...
    logCat(
        "file-add",
        `ADDED ${absFilePath}: ${hunks.length} hunks, type=${changeType}, mergedLines=${lines.length}, ranges=${ranges.length}, session=${sessionId ?? "none"}`,
    );
    mgr.scheduleSave();
...
export async function handleMissingFile(
    ...
    if (origContent) {
        ...
        logCat(
            "file-add",
            `handleMissingFile: ${absFilePath} — found original via ${source} (${origContent.length} chars), treating as deletion`,
        );
    ...
    try {
        ...
        if (!relPath.startsWith("..")) {
            ...
            logCat(
                "file-add",
                `handleMissingFile: ${absFilePath} — found via git show HEAD (${gitContent.length} chars)`,
            );
        ...
        logCat(
            "file-add",
            `handleMissingFile: ${absFilePath} — outside workspace (relPath=${relPath}), cannot git show`,
        );
    } catch (err) {
        logCat(
            "file-add",
            `handleMissingFile: ${absFilePath} — git show HEAD failed: ${(err as Error).message}`,
        );
    }

    logCat(
        "file-add",
        `handleMissingFile: ${absFilePath} — no original found (no snapshot, no existing review, no git), skipping`,
    );
}
...
export function getOriginalContent(
    ...
    if (existingOriginal !== undefined) {
        logCat(
            "file-add",
            `original source: existingReview for ${absFilePath} (${existingOriginal.length} chars)`,
        );
    ...
    if (snapshot !== undefined) {
        logCat(
            "file-add",
            `original source: PreToolUse snapshot for ${absFilePath} (${snapshot.length} chars)`,
        );
        return snapshot;
    }
    ...
    try {
        ...
        if (!relPath.startsWith("..")) {
            ...
            logCat(
                "file-add",
                `original source: git show HEAD for ${absFilePath} (${content.length} chars)`,
            );
            return content;
        }
        logCat(
            "file-add",
            `original source: file outside workspace (relPath=${relPath}), falling back to empty`,
        );
    } catch (err) {
        logCat(
            "file-add",
            `original source: git show HEAD failed for ${absFilePath}: ${(err as Error).message}, falling back to empty`,
        );
    }
