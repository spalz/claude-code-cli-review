...
export async function undoResolve(mgr: ReviewManagerInternal): Promise<void> {
    ...
    const fsPath = activeFile && hasUndoState(activeFile) ? activeFile : lastUndoFile;

    if (!fsPath || !hasUndoState(fsPath)) {
        logCat(
            "resolve",
            `ReviewManager.undoResolve: no undo state available (active=${activeFile}, lastGlobal=${lastUndoFile})`,
        );
    ...
    const snapshotHunkState = snapshot.hunks
        .map((h) => `${h.id}:${h.resolved ? (h.accepted ? "A" : "R") : "U"}`)
        .join(",");
    logCat(
        "resolve",
        `ReviewManager.undoResolve: restoring ${fsPath}, unresolved=${snapshot.hunks.filter((h) => !h.resolved).length}, snapshot hunks=[${snapshotHunkState}], mergedLines=${snapshot.mergedLines.length}`,
    );
    ...
    logCat(
        "resolve",
        `ReviewManager.undoResolve: END total=${(performance.now() - tUndo).toFixed(1)}ms`,
    );
}
...
export async function redoResolve(mgr: ReviewManagerInternal): Promise<void> {
    ...
    const fsPath = activeFile && hasRedoState(activeFile) ? activeFile : lastRedoFile;

    if (!fsPath || !hasRedoState(fsPath)) {
        logCat(
            "resolve",
            `ReviewManager.redoResolve: no redo state available (active=${activeFile}, lastGlobal=${lastRedoFile})`,
        );
    ...
    const redoHunkState = snapshot.hunks
        .map((h) => `${h.id}:${h.resolved ? (h.accepted ? "A" : "R") : "U"}`)
        .join(",");
    logCat(
        "resolve",
        `ReviewManager.redoResolve: snapshot hunks=[${redoHunkState}], allResolved=${allResolved}, mergedLines=${snapshot.mergedLines.length}`,
    );
    if (allResolved) {
        ...
        logCat(
            "resolve",
            `ReviewManager.redoResolve: restoring ${fsPath}, unresolved=${snapshot.hunks.filter((h) => !h.resolved).length}`,
        );
        restoreFromSnapshot(mgr, fsPath, snapshot);
        // Reveal first unresolved hunk from the restored snapshot
        const firstRange = snapshot.hunkRanges[0];
    ...
    logCat(
        "resolve",
        `ReviewManager.redoResolve: END total=${(performance.now() - tRedo).toFixed(1)}ms`,
    );
}
...
export function restoreFromSnapshot(
    ...
    if (!review) {
        logCat(
            "resolve",
            `ReviewManager.restoreFromSnapshot: re-creating review for ${fsPath}, unresolved=${unresolvedCount}`,
        );
    ...
    if (state.activeReviews.has(fsPath)) {
        logCat(
            "resolve",
            `ReviewManager.restoreFromSnapshot: updating existing review for ${fsPath}, unresolved=${unresolvedCount}`,
        );
    }
    review.hunks = JSON.parse(JSON.stringify(snapshot.hunks));
    ...
    const rangeDetail = (review as FileReview).hunkRanges
        .map((r) => `h${r.hunkId}@${r.removedStart}-${r.removedEnd}/${r.addedStart}-${r.addedEnd}`)
        .join(", ");
    logCat(
        "resolve",
        `ReviewManager.restoreFromSnapshot: hunks=[${hunkDetail}], ranges=[${rangeDetail}]`,
    );
