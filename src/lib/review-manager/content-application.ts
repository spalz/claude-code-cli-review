...
export async function applyContentViaEdit(
    ...
    if (editor.document.getText() === newContent) {
        ...
        logCat(
            "content",
            `applyContentViaEdit: END ${fname} (skipped), total=${(performance.now() - t0).toFixed(1)}ms`,
        );
        return;
    }
    ...
    try {
        ...
        if (oldStart === oldEnd && newStart === newEnd) {
            ...
            logCat(
                "content",
                `applyContentViaEdit: targeted edit lines ${oldStart}-${oldEnd} → ${newEnd - newStart} lines (of ${oldLines.length}→${newLines.length})`,
            );
            editSuccess = await editor.edit((eb) => eb.replace(editRange, replacement), {
                undoStopBefore: true,
                undoStopAfter: true,
            });
        }
        ...
        logCat(
            "content",
            `applyContentViaEdit: editor.edit()=${editSuccess}, ${(performance.now() - tEdit).toFixed(1)}ms`,
        );
        if (!editSuccess) {
            // Retry with full replace as fallback
            logCat(
                "content",
                `applyContentViaEdit: targeted edit failed, retrying with full replace`,
            );
            ...
            const retryRange = new vscode.Range(
                ...
                0,
                retryLastLine,
                doc.lineAt(retryLastLine).text.length,
            );
            editSuccess = await editor.edit((eb) => eb.replace(retryRange, newContent), {
                undoStopBefore: true,
                undoStopAfter: true,
            });
        ...
        if (editSuccess) {
            ...
            logCat(
                "content",
                `applyContentViaEdit: saveWithoutFormatting ${(performance.now() - tSave).toFixed(1)}ms`,
            );
            ...
            logCat(
                "content",
                `applyContentViaEdit: retry also failed for ${filePath}, falling back to fs.writeFileSync`,
            );
            fs.writeFileSync(filePath, newContent, "utf8");
    ...
    if (editSuccess) {
        ...
        if (editor.document.getText() !== newContent) {
            logCat(
                "content",
                `applyContentViaEdit: post-edit buffer mismatch, doing full replace fallback`,
            );
            setApplyingEdit(filePath, true);
            try {
                const doc = editor.document;
                ...
                await editor.edit(
                    (eb) =>
                        eb.replace(
                            new vscode.Range(0, 0, fl, doc.lineAt(fl).text.length),
                            newContent,
                        ),
    ...
    editor.selection = new vscode.Selection(
        cursorLine,
        savedSelection.active.character,
        cursorLine,
        savedSelection.active.character,
    );
    ...
    logCat(
        "content",
        `applyContentViaEdit: END ${fname}, total=${(performance.now() - t0).toFixed(1)}ms, editSuccess=${editSuccess}`,
    );
}
