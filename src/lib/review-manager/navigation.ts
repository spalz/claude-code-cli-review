...
export function navigateHunk(mgr: ReviewManagerInternal, delta: number): void {
    ...
    logCat(
        "navigation",
        `navigateHunk: ${oldIdx} → ${mgr.currentHunkIndex} (delta=${delta}, total=${ranges.length})`,
    );
...
export async function navigateFile(mgr: ReviewManagerInternal, delta: number): Promise<void> {
    ...
    logCat(
        "navigation",
        `navigateFile: ${current || "none"} (idx=${curIdx}) → ${files[newIdx]} (idx=${newIdx}), delta=${delta}, unresolvedFiles=${files.length}`,
    );
...
export async function reviewNextUnresolved(mgr: ReviewManagerInternal): Promise<void> {
    ...
    for (let i = 1; i <= total; i++) {
        ...
        if (state.activeReviews.has(files[idx])) {
            logCat(
                "navigation",
                `reviewNextUnresolved: from index ${startIdx} → opening index ${idx} (${files[idx]})`,
            );
...
export async function openFileForReview(
    ...
    logCat(
        "navigation",
        `openFileForReview: START ${fname}, hunks=${review.hunks.length}, unresolved=${review.unresolvedCount}, mergedLines=${review.mergedLines.length}`,
    );
    ...
    logCat(
        "navigation",
        `openFileForReview: wrote merged content to disk (${mergedContent.length} chars)`,
    );
    ...
    if (doc.getText() !== mergedContent) {
        logCat(
            "navigation",
            `openFileForReview: editor content stale for ${fname} (docLen=${doc.getText().length}, mergedLen=${mergedContent.length}), syncing via edit`,
        );
        setApplyingEdit(filePath, true);
        try {
            const lastLine = doc.lineCount - 1;
            const fullRange = new vscode.Range(0, 0, lastLine, doc.lineAt(lastLine).text.length);
            await editor.edit((eb) => eb.replace(fullRange, mergedContent), {
                undoStopBefore: true,
                undoStopAfter: true,
            });
            ...
            try {
                ...
                logCat(
                    "navigation",
                    `openFileForReview: revert failed for ${filePath}: ${(err as Error).message}`,
                );
            }
        } finally {
