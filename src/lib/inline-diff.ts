...
export function computeInlineChanges(
	...
	if (!removedLine) return { removedChanges: [], addedChanges: [{ start: 0, end: addedLine.length }] };
	if (!addedLine) return { removedChanges: [{ start: 0, end: removedLine.length }], addedChanges: [] };
	...
	for (let i = 0; i < tokens.length; i++) {
		if (!matched[i]) {
			...
			if (currentStart !== -1) {
				...
				changes.push({ start: currentStart, end: prevToken.offset + prevToken.text.length });
				currentStart = -1;
			}
		}
	}
...
export function computeHunkInlineChanges(
	...
	for (let i = 0; i < pairedCount; i++) {
		const { removedChanges, addedChanges } = computeInlineChanges(removedLines[i], addedLines[i]);
