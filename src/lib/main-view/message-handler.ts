...
import {
    readClaudeSettings,
    readClaudeSettingsScoped,
    writeClaudeSetting,
    writeClaudeRuntimeState,
    trustProject,
 ...
 */
export async function resolveClipboard(): Promise<
    { type: "text"; text: string } | { type: "image"; tmpPath: string } | null
> {
    if (process.platform === "darwin") {
        ...
        try {
            ...
            if (filePath) {
                log.log(
                    `clipboard: furl rejected (not a valid file path): "${filePath.slice(0, 80)}"`,
                );
            }
        } catch {
    ...
    switch (msg.type) {
        case "webview-ready":
            ...
            ctx.postMessage({
                ...
                terminalSettings: {
                    soundEnabled: termConfig.get<boolean>("soundEnabled", true),
                },
            });
        ...
        case "resume-claude-session": {
            ...
            if (existingPtyId !== null) {
                ...
                log.log(
                    `resume: session ${claudeSessionId.slice(0, 8)} is lazy, activating placeholder`,
                );
                ctx.postMessage({
        ...
        case "read-clipboard": {
            ...
            resolveClipboard().then((result) => {
                ...
                if (result.type === "text") {
                    log.log(
                        `read-clipboard: session #${clipSessionId}, text len=${result.text.length}, "${result.text.slice(0, 80)}"`,
                    );
                    ctx.ptyManager.writeToSession(
                        clipSessionId,
                        `\x1b[200~${result.text}\x1b[201~`,
                    );
                    ...
                    if (imageExts.some((ext) => textLower.endsWith(ext))) {
                        try {
                            ...
                            if (fs.existsSync(imgPath)) {
                                ...
                                const mimeType = textLower.endsWith(".png")
                                    ? "image/png"
                                    : "image/jpeg";
                            ...
                            log.log(
                                `read-clipboard: image register from text failed: ${(err as Error).message}`,
                            );
                        }
                    }
                } else {
                    ...
                    log.log(
                        `read-clipboard: session #${clipSessionId}, image at ${result.tmpPath}`,
                    );
                    ctx.ptyManager.writeToSession(
                        clipSessionId,
                        `\x1b[200~${result.tmpPath}\x1b[201~`,
                    );
                    // Enqueue image for binding when CLI outputs [Image #N]
                    try {
                        const base64 = fs.readFileSync(result.tmpPath).toString("base64");
                        const mimeType = result.tmpPath.endsWith(".png")
                            ? "image/png"
                            : "image/jpeg";
        ...
        case "set-claude-setting": {
            ...
            log.log(
                `set-claude-setting [${scope}]: ${msg.key as string} = ${JSON.stringify(msg.value)}`,
            );
