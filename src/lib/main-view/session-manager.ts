...
export class SessionManager {
    ...
    refreshClaudeSessions(): void {
        ...
        log.log(
            `refreshClaudeSessions: found ${sessions.length} sessions, ${archivedCount} archived`,
        );
    ...
    startNewClaudeSession(resumeId?: string, restoring?: boolean): void {
        ...
        state.refreshAll();
        log.log(
            `startNewClaudeSession: resume=${resumeId || "none"}, pty=${info.id}, ${Date.now() - t0}ms`,
        );
    ...
    isLazyPlaceholder(claudeId: string): boolean {
        return (
            this._restored &&
            this._allOpenClaudeIds.has(claudeId) &&
            this.findPtyByClaudeId(claudeId) === null
        );
    }
    ...
    sendOpenSessionIds(): void {
        ...
        log.log(
            `sendOpenSessionIds: [${openClaudeIds.map((id) => id.slice(0, 8)).join(", ")}], lazy=[${lazyClaudeIds.map((id) => id.slice(0, 8)).join(", ")}]`,
        );
    ...
    restoreSessions(): void {
        ...
        this.sendOpenSessionIds();
        log.log(
            `restoreSessions: ${ids.length} restored (1 active, ${ids.length - 1} lazy) in ${Date.now() - t0}ms`,
        );
    ...
    lazyResume(claudeId: string, placeholderPtyId: number): void {
        ...
        const info = this._ptyManager.createSession(`resume:${claudeId.slice(0, 8)}`, cmd);
