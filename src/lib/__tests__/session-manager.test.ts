...
function makeMemento(data: Record<string, unknown> = {}): { get: ReturnType<typeof vi.fn>; update: ReturnType<typeof vi.fn> } {
	const store = { ...data };
	return {
...
describe("constructor", () => {
	...
	it("handles missing workspace state gracefully", () => {
		messages = [];
		ptyIdCounter = 0;
		const mgr = new SessionManager("/ws", makePtyManager(), undefined, (msg) => messages.push(msg));
