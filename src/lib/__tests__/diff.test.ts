...
describe("parseUnifiedDiff", () => {
    it("parses a single hunk with removals and additions", () => {
        const diff = makeUnifiedDiff([
            { removed: ["old"], added: ["new"], origStart: 1, modStart: 1 },
        ]);
    ...
    it("splits hunks on context lines within a @@ block", () => {
        const diff = ["@@ -1,5 +1,5 @@", "-old1", "+new1", " context", "-old2", "+new2"].join("\n");
        const hunks = parseUnifiedDiff(diff);
        expect(hunks).toHaveLength(2);
    ...
    it("sub-hunks get correct positions (not header values)", () => {
        const diff = ["@@ -1,5 +1,5 @@", "-old1", "+new1", " context", "-old3", "+new3"].join("\n");
        const hunks = parseUnifiedDiff(diff);
        expect(hunks).toHaveLength(2);
    ...
    it("sub-hunks: deletion then insertion separated by context", () => {
        const diff = ["@@ -1,4 +1,3 @@", "-deleted", " context", "+inserted"].join("\n");
        const hunks = parseUnifiedDiff(diff);
        expect(hunks).toHaveLength(2);
    ...
    it("handles pure addition (only + lines)", () => {
        const diff = makeUnifiedDiff([
            { removed: [], added: ["line1", "line2"], origStart: 1, modStart: 1 },
        ]);
        const hunks = parseUnifiedDiff(diff);
        expect(hunks).toHaveLength(1);
    ...
    it("handles pure deletion (only - lines)", () => {
        const diff = makeUnifiedDiff([
            { removed: ["gone1", "gone2"], added: [], origStart: 1, modStart: 1 },
        ]);
        const hunks = parseUnifiedDiff(diff);
        expect(hunks).toHaveLength(1);
    ...
    it("sets correct origStart/modStart/origCount/modCount", () => {
        const diff = makeUnifiedDiff([
            { removed: ["a", "b"], added: ["c"], origStart: 5, modStart: 7 },
        ]);
        const hunks = parseUnifiedDiff(diff);
    ...
    it("assigns sequential hunk ids", () => {
        const diff = ["@@ -1,1 +1,1 @@", "-a", "+b", "@@ -5,1 +5,1 @@", "-c", "+d"].join("\n");
        const hunks = parseUnifiedDiff(diff);
