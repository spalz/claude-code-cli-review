...
describe("saveSessionName", () => {
    ...
    it("appends custom-title to .jsonl for CLI compatibility", () => {
        ...
        const jsonlWrite = mockFs.appendFileSync.mock.calls.find((c: unknown[]) =>
            (c[0] as string).endsWith("sess-123.jsonl"),
        );
...
describe("listSessions", () => {
    ...
    it("includes small files with custom name", () => {
        const userMsg = jsonl({ type: "user", message: { content: "test" } });
        setupSessionFiles([{ name: "small.jsonl", size: 100, mtime: 1000, content: userMsg }], {
    ...
    it("uses custom name over meta.title", () => {
        ...
        setupSessionFiles([{ name: "sess.jsonl", size: 5000, mtime: 1000, content: userMsg }], {
    ...
    it("custom-title in .jsonl overrides session-names.json", () => {
        ...
        setupSessionFiles([{ name: "sess.jsonl", size: 5000, mtime: 1000, content }], {
    ...
    it("forceIncludeIds bypasses messageCount filter", () => {
        setupSessionFiles([{ name: "empty.jsonl", size: 5000, mtime: 1000, content: "" }]);
...
describe("archiveSession / unarchiveSession", () => {
    ...
    it("creates file when it does not exist (unarchiveSession)", () => {
        ...
        expect(mockFs.writeFileSync).toHaveBeenCalledWith(ARCHIVED, expect.any(String), "utf8");
    });
});
...
describe("getSessionsDir", () => {
    it("encodes workspace path to project key", () => {
        ...
        expect(dir).toBe(
            path.join("/home/test", ".claude", "projects", "-Users-spals-projects-foo"),
        );
    });
});
