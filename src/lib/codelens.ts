...
export class ReviewCodeLensProvider implements vscode.CodeLensProvider {
    ...
    provideCodeLenses(document: vscode.TextDocument): vscode.CodeLens[] {
        ...
        for (const range of review.hunkRanges) {
            ...
            log.log(
                `CodeLens: hunk=${range.hunkId} removed=${range.removedStart}-${range.removedEnd} added=${range.addedStart}-${range.addedEnd} lensLine=${lensLine} docLines=${document.lineCount}`,
            );
        ...
        log.log(
            `CodeLens.provide: ${document.uri.fsPath.split("/").pop()}, ${lenses.length / 2} hunks, ${(performance.now() - t0).toFixed(1)}ms`,
        );
        return lenses;
    }
}
