...
import {
    ...
    setWorkspacePath,
    setExtensionPath,
    setGetActiveReviewHandler,
    setGetActiveSessionHandler,
    setPostWebviewMessageHandler,
} from "./lib/server";
...
export function activate(context: vscode.ExtensionContext): void {
    ...
    try {
        ...
        ptyManager.setHandlers(
            ...
            (sessionId, code, ageMs) => {
                ...
                if (ageMs !== undefined && ageMs < 5000 && code === 0) {
                    log.log(
                        `PTY quick exit: session=${sessionId}, age=${ageMs}ms — session likely completed`,
                    );
            ...
            setTimeout(async () => {
                try {
                    await vscode.commands.executeCommand("vscode.moveViews", {
                        viewIds: ["ccr.main"],
                        destinationId: "workbench.panel.auxiliarybar",
                    });
        ...
        const cmds: Array<[string, (...args: any[]) => any]> = [
            ...
            [
                ...
                () => {
                    ...
                    log.logCat(
                        "resolve",
                        `ccr.undo COMMAND FIRED — activeFile=${fp?.split("/").pop()}, hasUndo=${fp ? hasUndoState(fp) : "no-editor"}, hasRedo=${fp ? hasRedoState(fp) : "no-editor"}`,
                    );
            ...
            [
                ...
                () => {
                    ...
                    log.logCat(
                        "resolve",
                        `ccr.redo COMMAND FIRED — activeFile=${fp?.split("/").pop()}, hasUndo=${fp ? hasUndoState(fp) : "no-editor"}, hasRedo=${fp ? hasRedoState(fp) : "no-editor"}`,
                    );
        ...
        context.subscriptions.push(
            vscode.window.onDidChangeActiveTextEditor((editor) => {
                ...
                if (reviewManager?.isTransitioning) {
                    ...
                    setTimeout(() => {
                        ...
                        vscode.commands.executeCommand(
                            ...
                            !!rev || deferredHasHistory,
                        );
                        log.logCat(
                            "focus",
                            `onTabSwitch DEFERRED: ${fp.split("/").pop()} — review=${!!rev}, hasUndo=${hasUndoState(fp)}, hasRedo=${hasRedoState(fp)}, setting ccr.activeFileInReview=${!!rev || deferredHasHistory}`,
                        );
                        if (rev) {
                            ...
                            if (currentEditor.document.getText() === merged) {
                                ...
                                log.log(
                                    `onTabSwitch deferred: content mismatch for ${fp}, self-healing`,
                                );
                ...
                if (editor) {
                    ...
                    log.logCat(
                        "focus",
                        `onTabSwitch: ${fname} — review=${!!review}, hasUndo=${undoAvail}, hasRedo=${redoAvail}, setting ccr.activeFileInReview=${ctxValue}`,
                    );
                    vscode.commands.executeCommand(
                        "setContext",
                        "ccr.activeFileInReview",
        ...
        setPostWebviewMessageHandler((msg) =>
            mainView.postMessageDirect(msg as ExtensionToWebviewMessage),
        );
