...
(function () {
    ...
    var tsSoundEnabled = document.getElementById("ts_soundEnabled");
    if (tsSoundEnabled) {
        tsSoundEnabled.addEventListener("change", function () {
            send("set-terminal-setting", { key: "soundEnabled", value: this.checked });
        });
    }

    window.updateTerminalSettings = function (data) {
        if (!data) return;
        if (tsSoundEnabled && data.soundEnabled !== undefined)
            tsSoundEnabled.checked = !!data.soundEnabled;
    };
    ...
    function updateControlStates() {
        ...
        var settingsIds = Object.keys(csSelectMap)
            .concat(Object.keys(csToggleMap))
            .concat(Object.keys(csTextMap));
